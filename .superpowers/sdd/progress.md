...
- Task 10: implementation complete (pending final release commit). Closed the seventh- and eighth-review findings: Unicode-safe password changes, shared non-blocking Argon2 admission, bounded auth payloads, cross-tab CSRF recovery, exact break-glass role validation, atomic user patch/audit storage, replicated authentication-config digest checks, immutable public-dashboard service-token permissions, human-auth URL token suppression, trusted fence-adapter snapshot execution, final-proof-bound two-stage GTID/RPO acceptance, and MySQL password removal from process arguments.
- Task 10 verification: 835 Python tests passed / 1 skipped, 25 browser behavior tests passed, all 20 JavaScript files passed syntax checks, all 14 shell scripts passed syntax checks, and the independent security re-review found no remaining substantive Critical or Important code-level issue. The new executable RPO acceptance verifier is included in the pending release commit.
- Release gate: the current public endpoint is HTTP-only. Keep `SMART_BAMBOO_HUMAN_AUTH_ENABLED=0` until a real FQDN, trusted TLS certificate, Secure-cookie checks, provider-backed fencing adapter, live MySQL replication drill, and backup restore drill all pass.
