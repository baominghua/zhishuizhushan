...
export function AttachmentsPage() {
  ...
  return <div className="standard-page ledger-page">
    ...
    <LedgerSummaryStrip metrics={[
      { label: "文件总数", value: query.data?.total ?? 0, detail: "当前权限范围" },
      { label: "当前页容量", value: formatBytes((query.data?.items ?? []).reduce((sum, item) => sum + item.sizeBytes, 0)), detail: "受控文件", tone: "active" },
      { label: "已关联业务", value: (query.data?.items ?? []).filter((item) => item.linkCount > 0).length, detail: "本页文件" },
      { label: "待关联", value: (query.data?.items ?? []).filter((item) => item.linkCount === 0).length, detail: "本页文件", tone: "warning" },
    ]} />
