...
export function AssetViewerPage() {
  ...
  const trajectoryQuery = useQuery({
    ...
    queryFn: () => api.sceneTrajectory(sceneId),
    enabled: Boolean(sceneId && asset?.trajectoryAvailable && pointInformationMode === "trajectory"),
    staleTime: 5 * 60_000,
    retry: 1,
  });
              ...
              {mode === "2d" ? (
                ...
                <Suspense fallback={<div className="map-service-state">正在启动 COPC 流式点云引擎</div>}>
                  <CopcPointCloudViewer
                    ...
                    bounds={asset.bounds}
                    featureCollection={showForestBlocks ? boundaryFeatures : EMPTY_FEATURES}
                    trajectory={pointInformationMode === "trajectory" ? trajectoryQuery.data : undefined}
                    settings={settings}
                    quality={quality}
                    homeRequest={homeRequest}
                    zoomRequest={zoomRequest}
