...
function CoverageConfirmation({ asset, onCancel, onSaved }: { asset: ImageryAsset; onCancel: () => void; onSaved: (record: ImageryAsset) => void }) {
  ...
  const confirm = useMutation({ mutationFn: () => api.confirmImageryCoverage(asset.id, blocks.map((block) => block.code)), onSuccess: onSaved });
  ...
  return <div className="coverage-review">
    <section className="coverage-review-heading"><span className="eyebrow">空间分析完成</span><h3>确认实际覆盖林班</h3><p>系统按有效像素或点云分片范围计算交叠；建议结果不会直接写入正式关系，确认后才生效。</p></section>
    ...
    <div className="relation-toolbar"><div><strong>最终关联林班<em>*</em></strong><small>跨林班成果只建一份档案，可确认多个林班。</small></div><button className="button secondary" type="button" onClick={() => setSelectorOpen(true)}><MapPinned aria-hidden="true" />补充林班</button></div>
    <BlockChips blocks={blocks} onRemove={(code) => setBlocks((items) => items.filter((item) => item.code !== code))} />
    {confirm.error ? <p className="form-error">{confirm.error.message}</p> : null}
    <div className="form-actions"><button className="button secondary" type="button" onClick={onCancel}>稍后确认</button><button className="button primary" disabled={confirm.isPending || !blocks.length} onClick={() => confirm.mutate()}>{confirm.isPending ? "正在写入关系" : `确认关联 ${blocks.length} 个林班`}</button></div>
...
function ImageryEditForm({ initial, onCancel, onSaved }: { initial: ImageryAsset; onCancel: () => void; onSaved: (record: ImageryAsset) => void }) {
  ...
  const mutation = useMutation({
    mutationFn: async (form: HTMLFormElement) => {
      const data = new FormData(form);
      await api.updateImageryAsset(initial.id, { name: field(data, "name"), missionId: field(data, "missionId"), capturedAt: field(data, "capturedAt"), resolution: field(data, "resolution"), linkedBlockCodes: blocks.map((block) => block.code) });
      return api.confirmImageryCoverage(initial.id, blocks.map((block) => block.code));
    },
    onSuccess: onSaved,
  });
  const submit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); mutation.mutate(event.currentTarget); };
  return <><form className="entity-form" onSubmit={submit}><fieldset className="form-section"><legend>成果元数据</legend><div className="form-grid"><label><span>成果名称<em>*</em></span><input name="name" required defaultValue={initial.name} /></label><label><span>成果类型</span><input readOnly value={TYPE_LABELS[assetType(initial)]} /></label><label><span>无人机任务</span><input name="missionId" defaultValue={initial.missionId} /></label><label><span>采集时间</span><input name="capturedAt" type="datetime-local" defaultValue={localTime(initial.capturedAt)} /></label>{assetType(initial) !== "pointcloud" ? <label><span>标称分辨率</span><input name="resolution" defaultValue={initial.resolution} /></label> : null}</div></fieldset><fieldset className="form-section"><legend>正式林班关系</legend><div className="relation-toolbar"><div><strong>覆盖林班<em>*</em></strong><small>保存后同步替换成果与林班的正式关联关系。</small></div><button className="button secondary" type="button" onClick={() => setSelectorOpen(true)}><MapPinned aria-hidden="true" />选择林班</button></div><BlockChips blocks={blocks} onRemove={(code) => setBlocks((items) => items.filter((item) => item.code !== code))} /></fieldset>{mutation.error ? <p className="form-error">{mutation.error.message}</p> : null}<div className="form-actions"><button className="button secondary" type="button" onClick={onCancel}>取消</button><button className="button primary" disabled={mutation.isPending || !blocks.length}>{mutation.isPending ? "正在保存" : "保存元数据与关系"}</button></div></form><ForestBlockSelector open={selectorOpen} onClose={() => setSelectorOpen(false)} onSelect={(block) => setBlocks((items) => items.some((item) => item.code === block.code ? true : false) ? items : [...items, block])} /></>;
}
...
function ImageryDetail({ record, canPublish, publishing, onPublish }: { record: ImageryAsset; canPublish: boolean; publishing: boolean; onPublish: () => void }) {
  ...
  return <div className="detail-stack imagery-detail">
    ...
    {!is3d && isPublished(record) ? <a className="button primary" href={`/v2/asset-viewer?sceneId=${encodeURIComponent(record.id)}&mode=2d`} target="_blank" rel="noreferrer"><Globe2 aria-hidden="true" />在独立二维窗口打开</a> : null}
