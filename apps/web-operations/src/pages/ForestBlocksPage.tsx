...
function ForestBlockDetail({ record, canRollback, canEstimate, estimatePending, estimateError, onEstimate, onRestored }: { record: ForestBlockRecord; canRollback: boolean; canEstimate: boolean; estimatePending: boolean; estimateError: Error | null; onEstimate: () => void; onRestored: () => Promise<void> }) {
  ...
  return <div className="detail-stack">
    ...
    <section className="detail-group moso-inventory-card">
      <div className="moso-inventory-heading"><div><span>AI 科研试算</span><h3>毛竹资源量</h3></div><div className="moso-inventory-actions">{mosoInventory && <a className="button secondary" href={`/v2/ai/moso-inventory-sandbox?blockId=${encodeURIComponent(record.id)}`} target="_blank" rel="noreferrer"><MapPin aria-hidden="true" />估算沙盘</a>}<button className="button secondary" type="button" disabled={!canEstimate || estimatePending || !record.geometry} onClick={onEstimate}><Sparkles aria-hidden="true" />{estimatePending ? "分析中" : mosoInventory ? "更新试算" : "生成试算"}</button></div></div>
      {mosoInventory ? <>
