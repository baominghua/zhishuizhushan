...
export function DroneMissionsPage() {
  ...
  return <div className="standard-page ledger-page drone-page">
    <section className="page-heading ledger-heading">
      ...
      <div className="heading-actions"><a className="button secondary" href={`/api/v2/drone/missions-export.csv?q=${encodeURIComponent(q)}&status=${encodeURIComponent(status)}`}><Download aria-hidden="true" />导出</a><button className="button secondary" type="button" onClick={() => ledger.refetch()}><RefreshCw aria-hidden="true" />刷新</button><button className="button primary" type="button" disabled={!canManage} onClick={() => setCreating(true)}><Plus aria-hidden="true" />新建任务</button></div>
    </section>
...
function MissionDetail({ record, permissions, pending, error, onAction }: { record: DroneMission; permissions: { assign: boolean; start: boolean; upload: boolean; review: boolean }; pending: boolean; error: Error | null; onAction: (action: string, payload: DroneMissionActionPayload) => void }) {
  ...
  return <div className="domain-detail"><section className="detail-group"><div className="detail-title-row"><h3>任务信息</h3><StatusBadge status={record.status} /></div><dl><Fact label="任务编号" value={record.missionNo} /><Fact label="任务类型" value={TYPE_LABELS[record.missionType]} /><Fact label="执行无人机" value={`${record.deviceName} · ${record.deviceCode}`} /><Fact label="飞手" value={record.pilotName} /><Fact label="航线" value={record.routeName} /><Fact label="计划窗口" value={`${formatDateTime(record.plannedStartAt)} 至 ${formatDateTime(record.plannedEndAt)}`} /><Fact label="实际飞行" value={record.actualStartAt ? `${formatDateTime(record.actualStartAt)} 至 ${record.actualEndAt ? formatDateTime(record.actualEndAt) : "进行中"}` : "尚未起飞"} /><Fact label="任务目标" value={record.objective} /></dl><div className="relation-chips read-only">{record.blocks.map((item) => <span key={item.code}><strong>{item.code}</strong><small>正式目标林班</small></span>)}</div></section><MissionActionCard record={record} permissions={permissions} pending={pending} onAction={onAction} />{error && <p className="form-error">{error.message}</p>}<section className="detail-group"><h3>成果资产</h3>{controlledAssets.length ? <div className="asset-list">{controlledAssets.map((asset) => <a href={asset.downloadUrl || undefined} target="_blank" rel="noreferrer" key={asset.id}><FileUp aria-hidden="true" /><span><strong>{asset.originalName}</strong><small>{formatBytes(asset.sizeBytes)} · SHA-256 {asset.sha256.slice(0, 12)}</small></span></a>)}</div> : legacyAssets.length ? <div className="asset-list">{legacyAssets.map((url) => <a href={url} target="_blank" rel="noreferrer" key={url}><FileUp aria-hidden="true" /><span><strong>历史外部成果</strong><small>{url}</small></span></a>)}</div> : <div className="action-unavailable"><FileUp aria-hidden="true" /><span><strong>暂无成果资产</strong><small>完成飞行后从统一附件中心提交正射影像、视频或模型成果。</small></span></div>}</section><section className="detail-group"><h3>任务时间线</h3><div className="domain-timeline">{record.timeline.map((item) => <article key={item.id}><i /><div><strong>{actionLabel(item.action)}</strong><small>{item.actor} · {formatDateTime(item.createdAt)}</small><p>{item.note || `${STATUS_LABELS[item.fromStatus as DroneMissionStatus] || "创建"} → ${STATUS_LABELS[item.toStatus as DroneMissionStatus] || item.toStatus}`}</p></div></article>)}</div></section></div>;
}
...
function actionLabel(action: string) { return ({ create: "建立任务", edit: "修改计划", assign: "派发任务", start: "开始飞行", "upload-result": "提交成果", review: "成果复核", return: "退回处理", complete: "完成归档", cancel: "取消任务" } as Record<string, string>)[action] || action; }
