...
export function MosoInventorySandboxPage() {
  ...
  const detectedCandidateCount = estimate?.crownCandidateLocationCount ?? candidates.length;
  const hasCompleteCandidateSet = Boolean(estimate?.crownCandidateLocationsComplete) && candidates.length === detectedCandidateCount;
  const extrapolatedCrownCount = estimate?.crownEquivalentCount.value ?? detectedCandidateCount;
  const extrapolatedCandidateDelta = Math.max(0, extrapolatedCrownCount - detectedCandidateCount);
    ...
    <QueryState loading={blockQuery.isLoading} error={blockQuery.error}>{!record || !estimate ? <div className="moso-sandbox-empty"><ScanSearch /><h2>该林班暂无试算结果</h2><p>请先在林班台账执行“生成试算”，完成后即可查看真实影像沙盘。</p><a className="button primary" href="/v2/resources/forest-blocks">进入林班台账</a></div> : <>
      <div className="moso-sandbox-layout">
        ...
        <main className="moso-sandbox-stage"><div className="moso-stage-heading"><div><span>真实影像沙盘</span><strong>{record.blockCode}</strong></div><div><i />已标注竹冠候选 {formatNumber(candidates.length)} 个{hasCompleteCandidateSet ? " · 全部" : ""}</div></div><div className="moso-stage-canvas real-imagery">
          ...
          <div className="moso-stage-legend"><span><i className="boundary" />青色林班边界</span><span><i className="candidate" />琥珀菱形候选点</span><small>深色与白色双描边，缩放后仍可辨</small></div><div className="moso-stage-caption"><Crosshair /><span>{candidates.length ? `${hasCompleteCandidateSet ? "已显示本次检测的全部候选点" : "当前记录仅保存部分候选点"}；候选点是影像冠峰，不等同于外业确认的单株坐标。` : "该历史试算只保存汇总结果，重新试算后即可在真实影像上显示全部竹冠候选点。"}</span></div>
        ...
        <aside className="moso-summary-panel"><div className="moso-panel-heading"><span>试算结果</span><strong>资源汇总</strong></div><Metric label="模型估算株数" value={formatNumber(estimate.resourceStock.value)} unit="株" detail={`${formatNumber(estimate.resourceStock.lower)}–${formatNumber(estimate.resourceStock.upper)} 株 · 非逐株清点`} /><div className="moso-summary-grid"><Metric label="有坐标影像候选" value={formatNumber(detectedCandidateCount)} unit="个" detail={hasCompleteCandidateSet ? "地图已全部标注" : "需重新试算补齐坐标"} /><Metric label="林班面积" value={formatNumber(estimate.blockArea.value, 2)} unit={estimate.blockArea.unit} /></div><div className="moso-summary-grid"><Metric label="密度模型值" value={formatNumber(estimate.stemDensity.value, 1)} unit={estimate.stemDensity.unit} /><Metric label="冠层覆盖" value={formatNumber(estimate.canopyClosure.value, 1)} unit="%" /></div><Metric label="地上生物量试算" value={formatNumber(estimate.abovegroundBiomass.value, 1)} unit="t" /><section className="moso-confidence"><div><span>结果置信度</span><strong>{estimate.confidence.level}</strong></div><div className="moso-confidence-track"><i style={{ width: `${Math.round(estimate.confidence.score * 100)}%` }} /></div><small>{Math.round(estimate.confidence.score * 100)}% · 科研试算</small></section><dl className="moso-evidence-list"><div><dt>影像分辨率</dt><dd>{numberValue(estimate.imageryEvidence, "nativeResolutionM") || "—"} m</dd></div><div><dt>影像覆盖率</dt><dd>{numberValue(estimate.imageryEvidence, "imageCoveragePct") || "—"}%</dd></div><div><dt>已显示候选点</dt><dd>{formatNumber(candidates.length)} 个</dd></div><div><dt>覆盖外推补量</dt><dd>{extrapolatedCandidateDelta ? `+${formatNumber(extrapolatedCandidateDelta)} 株` : "无需外推"}</dd></div><div><dt>估算时间</dt><dd>{formatDateTime(estimate.estimatedAt)}</dd></div></dl></aside>
